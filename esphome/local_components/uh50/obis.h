...
inline const char *get_obis_name(const char *obis_code) {
  for (unsigned int i = 0; i < sizeof(OBIS_DICT) / sizeof(OBIS_DICT[0]); i++) {
...
// Copy at most dst_size-1 bytes and always null-terminate, guarding the
// fixed-size destination fields against oversized telegram tokens.
inline void copy_field(char *dst, const char *src, size_t src_len, size_t dst_size) {
...
inline void parse_obis(const char *obis_string, OBISData *parsed_data, int *parsed_count) {
  ...
  *parsed_count = 0;
  ...
  while (*ptr != '\0' && *parsed_count < MAX_OBIS_CODES) {
    ...
    while (*ptr == 0xa || *ptr == 0x0d)
      ptr++;  // Skip newlines
    const char *obis_code_end = strchr(ptr, '(');
    if (obis_code_end == NULL)
      break;

    copy_field(data->obis_code, ptr, (size_t) (obis_code_end - ptr), MAX_OBIS_LENGTH);
    ...
    copy_field(data->value, value_start, (size_t) (value_end - value_start), MAX_VALUE_LENGTH);

    char *unit_start = strchr(data->value, '*');
    if (unit_start != NULL) {
      *unit_start = '\0';
      copy_field(data->unit, unit_start + 1, strlen(unit_start + 1), MAX_UNIT_LENGTH);
    } else {
      data->unit[0] = '\0';
    }

    const char *name = get_obis_name(data->obis_code);
    copy_field(data->name, name, strlen(name), MAX_NAME_LENGTH);
